...
parser.add_argument('--path', help='HTTP path to use for the initial request', metavar="fake.domain")
...
try:
    if args.v is not None:
        location = Location(args.loc, args.loc_ip, debug_log=(args.v == 2))
    else:
        location = Location(args.loc, args.loc_ip)
...
etc_hosts = None
if args.browser:
    browser_domain = '127.0.0.1' if args.domain is None else args.domain
    try:
        location.browser_onto(container, fqdn=browser_domain, path=args.path)
        ...
        if os.geteuid() !=0:
            ...
            with open('/etc/hosts', 'a') as f:
                f.write('127.0.0.1 %s\n' % browser_domain)
            location.browser_onto(container, fqdn=browser_domain)
