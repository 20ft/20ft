...
from tfnz import last_image, get_external_ip
...
def main():
    ...
    parser.add_argument('--public', action="store_true", help='Bind to the external ip')
    parser.add_argument('--offset', help='Offset to apply to port numbers', metavar='+n', default=0)
    ...
    if not args.browser:
        ...
        for exposed in ports.keys():
            ...
            port = int(exposed[:-4])
            lp = port
            if os.geteuid() != 0:  # not root or sudo, find a free port if port number is too low
                lp = (port + int(args.offset)) if (port + int(args.offset)) > 1024 else None
            container.attach_tunnel(port, localport=lp, bind=ip)
    ...
    if args.browser:
        ...
        try:
            container.attach_browser(fqdn=browser_domain, path=args.path)
            ...
            if os.geteuid() != 0:
                ...
                print("Run this command as the superuser to have it fixed automatically.", file=sys.stderr)
                exit(8)
            else:
    ...
    if json_file is not None:
        ...
        root['tunnels'] = {tnl.port: tnl.localport for tnl in container.all_tunnels()}  # ints can't be used as keys :(
        root['ip'] = ip
        root['pid'] = os.getpid()
        try:
